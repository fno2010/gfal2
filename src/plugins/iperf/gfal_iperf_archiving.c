...
int gfal_plugin_iperf_archive_poll(plugin_handle plugin_data, const char* url, GError** err)
{
    // Archiving errno
    char arg_buffer[64];
    gfal_plugin_iperf_get_value(url, "archiving_errno", arg_buffer, sizeof(arg_buffer));
    int archiving_error = gfal_plugin_iperf_get_int_from_str(arg_buffer);
    ...
    if (!archiving_started) {
        ...
        gfal_plugin_iperf_get_value(url, "archiving_time", arg_buffer, sizeof(arg_buffer));
        time_t *archiving_end = g_new0(time_t, 1);
        *archiving_end = time(NULL) + gfal_plugin_iperf_get_int_from_str(arg_buffer);
    ...
    if (archiving_end == NULL || *archiving_end <= time(NULL)) {
        if (archiving_error) {
            gfal_plugin_iperf_report_error(strerror(archiving_error), archiving_error, err);
...
int gfal_plugin_iperf_archive_poll_list(plugin_handle plugin_data, int nbfiles, const char* const* urls, GError** errors)
{
    ...
    for (i = 0; i < nbfiles; ++i) {
        r = gfal_plugin_iperf_archive_poll(plugin_data, urls[i], &(errors[i]));
        if (r > 0) {
            ++terminal_count;
        }
        else if (r < 0) {
