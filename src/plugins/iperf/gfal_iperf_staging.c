    ...
    GError **err)
{
    ...
    // Bring online errno
    gfal_plugin_iperf_get_value(url, "staging_errno", arg_buffer, sizeof(arg_buffer));
    int staging_errno = gfal_plugin_iperf_get_int_from_str(arg_buffer);

    // Polling time
    gfal_plugin_iperf_get_value(url, "staging_time", arg_buffer, sizeof(arg_buffer));
    time_t *staging_end = g_new0(time_t, 1);
    *staging_end = time(NULL) + gfal_plugin_iperf_get_int_from_str(arg_buffer);
    ...
    if (tsize > 36) {
        ...
        g_strlcpy(token, "iperf-token", tsize);
    }

    // Now, if remaining is <= 0, or blocking call, we are done
    if (*staging_end <= time(NULL) || !async) {
        if (staging_errno) {
            gfal_plugin_iperf_report_error(strerror(staging_errno), staging_errno, err);
            return -1;
        }
        return 1;
    }
...
int gfal_plugin_iperf_bring_online_v2(plugin_handle plugin_data, const char *url, const char *metadata,
    ...
    GError **err)
{
    return gfal_plugin_iperf_bring_online(plugin_data, url, pintime, timeout, token, tsize, async, err);
}
    ...
    const char *url, const char *token, GError **err)
{
    ...
    if (staging_end == NULL || *staging_end <= time(NULL)) {
        ...
        gfal_plugin_iperf_report_error("Not ready", EAGAIN, err);
    }
...
{
    char arg_buffer[64];
    gfal_plugin_iperf_get_value(url, "release_errno", arg_buffer, sizeof(arg_buffer));
    int release_errno = gfal_plugin_iperf_get_int_from_str(arg_buffer);

    if (release_errno) {
        gfal_plugin_iperf_report_error(strerror(release_errno), release_errno, err);
        return -1;
    }
    return 0;
}
    ...
    size_t tsize, int async, GError **err)
{
    ...
    for (i = 0; i < nbfiles; ++i) {
        r = gfal_plugin_iperf_bring_online(plugin_data, urls[i], pintime, timeout, token, tsize, async, &(err[i]));
        if (r > 0)
            ++terminal_count;
    }
...
{
    return gfal_plugin_iperf_bring_online_list(plugin_data, nbfiles, urls, pintime, timeout, token, tsize, async, err);
}
    ...
    int nbfiles, const char *const *urls, const char *token, GError **err)
{
    ...
    for (i = 0; i < nbfiles; ++i) {
        r = gfal_plugin_iperf_bring_online_poll(plugin_data, urls[i], token, &(err[i]));
        if (r > 0) {
            ++terminal_count;
        }
        else if (r < 0) {
    ...
    const char *const *urls, const char *token, GError **err)
{
    int i;
    for (i = 0; i < nbfiles; ++i) {
        gfal_plugin_iperf_release_file(plugin_data, urls[i], token, &(err[i]));
    }
    return 1;
}
